...
### Set parameters
model.choice <- 'FIRE_CSO_FREQ'
var.list <- c('company', 'channel', 'yearsInsured', 'stateRisk', 'ContStockOtherSI', 
              'suncorp_household_score', 'policyPremiscnt', 'occ_bld_fire', 'locality', 
              'CONTENT_FIRE_FREQ', 'target')
cat.var.list <- c('company', 'channel', 'stateRisk', 'occ_bld_fire', 'locality', 'CONTENT_FIRE_FREQ')
train.prop <- 0.8
